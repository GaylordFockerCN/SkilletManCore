...
public class EpicFightGuideBookItem extends SimpleDescriptionFoilItem {
    ...
    public static void addTranslation(SMCLangGenerator generator){
        ...
        generator.add(SMCRegistrateItems.EPIC_FIGHT_GUIDE_BOOK.get().key(2), "学习闪避类技能后，按下§6[%s]§r可进行§a闪避§r。若学习§6[完美闪避显示]§r，则在敌人攻击的瞬间闪避后，会留下残影和播放音效。§c看准时机闪避很关键，看准敌人出招再闪避！趁敌人攻击的后摇再反击！");
        generator.add(SMCRegistrateItems.EPIC_FIGHT_GUIDE_BOOK.get().key(3), "学习防御类技能后，按下[鼠标右键]可进行格挡。若学习[招架]，则在敌人攻击的瞬间成功招架后，会播放特殊动画和播放音效。§c看准时机格挡很关键，看准敌人出招再格挡！趁敌人攻击的后摇再反击！");
        generator.add(SMCRegistrateItems.EPIC_FIGHT_GUIDE_BOOK.get().key(4), "本整合包进行了§6群怪优化§r，聚集在一起的怪会轮流攻击。");
        generator.add(SMCRegistrateItems.EPIC_FIGHT_GUIDE_BOOK.get().key(5), "不同的武器会有不同的攻击模板，快去抽取强力武器吧！具体连招可以看看武器的物品说明！传说级武器键位在按键绑定中的[无坚不摧按键]进行修改。");
